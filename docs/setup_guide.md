...
4. [Создание шаблона ВМ в Proxmox VE](#4-создание-шаблона-вм-в-proxmox-ve)
5. [Запуск установки кластера](#5-запуск-установки-кластера)
...
Для быстрой настройки проекта используйте скрипт `setup.sh`:

```bash
# Перейдите в корень проекта
cd /workspace

# Запустите полную настройку (проверка зависимостей + копирование конфигов + инициализация Terraform)
./scripts/setup.sh -e dev -c proxmox -t simple

# Или только проверка зависимостей
./scripts/setup.sh -a check

# Только копирование конфигурационных файлов
./scripts/setup.sh -a setup
```

Скрипт автоматически:
- Проверит наличие всех необходимых зависимостей (Terraform, Ansible, kubectl, SSH)
- Скопирует пример конфигурации в нужную директорию
- Инициализирует Terraform
- Покажет дальнейшие шаги

**После выполнения скрипта перейдите к разделу [4. Создание шаблона ВМ](#4-создание-шаблона-вм-в-proxmox-ve)**

---

## 2. Ручная установка

Если вы предпочитаете ручную настройку или автоматический скрипт не работает, следуйте инструкциям ниже.

Подробные инструкции по установке всех зависимостей см. в документе [docs/INSTALL.md](INSTALL.md).
...
- **k3sup** (опционально, для альтернативного метода установки)
- **SSH** клиент и ключи

### 2.2. Быстрая установка зависимостей (Ubuntu/Debian)
...
### 3.1. Структура файлов переменных
